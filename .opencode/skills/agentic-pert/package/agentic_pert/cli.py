...
@app.command()
def plan(
    task: str = typer.Argument(..., help="High-level task for decomposition"),
    output: Literal["json", "mermaid"] = typer.Option("json", "--output", "-o"),
    input_file: str | None = typer.Option(
        None,
        "--input-file",
        help="File with structured planner JSON (when an external LLM has already run)",
    ),
) -> None:
    """Generate a PERT/CPM plan from a task (via planner JSON)."""

    if not input_file:
        typer.echo("Provide --input-file with planner JSON. Suggested prompt:\n")
