...
class Visualizer:
    ...
    def _to_mermaid_classic_pert(self) -> str:
        ...
        start_event = new_event("Start", 0.0, "start")
        ...
        for task in self.plan.tasks:
            ...
            end_node = new_event(f"Event {task.id}", timings.early_finish, "event")
        ...
        if len(terminal_tasks) == 1:
            ...
            final_join = new_event("Final convergence", final_join_time, "join")
            for task in terminal_tasks:
                lines.append(f"  {task_end_events[task.id]} -. dummy · 0h .-> {final_join}")

        final_event = new_event("Finish", self.plan.total_duration, "finish")
    ...
    def _to_mermaid_pert_gantt(self, milestone_label: Optional[str] = None) -> str:
        ...
        lines.append(
            f"  {self._sanitize_mermaid_gantt_label(milestone_label or 'Plan finish')} :milestone, plan_finish, "
